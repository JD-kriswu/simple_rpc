...
#ifndef _TENCENT_PBRPC_SERVICE_POOL_H_
#define _TENCENT_PBRPC_SERVICE_POOL_H_
...
public:
    ...
    void LatestStats(int slot_count, tencent::pbrpc::builtin::MethodStat* stat_out)
    {
        SCHECK(stat_out != NULL);
        stat_out->Clear();
        StatSlot stat;
...
public:
    ...
    void LatestStats(int method_id, int slot_count, tencent::pbrpc::builtin::MethodStat* stat_out)
    {
        SCHECK(method_id >=0 && method_id < _method_count);
        _method_boards[method_id].LatestStats(slot_count, stat_out);
    }

    void LatestStats(int64 expect_period, tencent::pbrpc::builtin::ServiceStat* stat_out)
    {
        SCHECK(stat_out != NULL);
        stat_out->Clear();
        int64 real_period;
        ...
        for (int i = 0; i < _method_count; ++i) {
            tencent::pbrpc::builtin::MethodStat* method_stat = stat_out->add_method_stats();
...
public:
    ...
    void ListService(tencent::pbrpc::builtin::ListServiceResponse* response)
    {
...
private:
    ...
    void AddFileDescriptor(tencent::pbrpc::builtin::ListServiceResponse* response,
...
private:
    ...
    TENCENT_PBRPC_DISALLOW_EVIL_CONSTRUCTORS(ServicePool);
}; // class ServicePool

} // namespace pbrpc
} // namespace tencent

#endif // _TENCENT_PBRPC_SERVICE_POOL_H_
